...
import { useFetchCategoriesQuery, useFetchMunicipalitiesQuery, useFetchObjectTypesQuery } from "@/store/api/TouristObjectApi";
...
const Reports = () => {
  ...
  const [modalFormat, setModalFormat] = useState<"pdf" | "csv" | "xlsx">("pdf");
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Select
              ...
              onChange={(selected) =>
                setFilters({ ...filters, objectTypeIds: selected.map((s) => s.value) })
              }
            />
            <Select
              isMulti
              ...
              classNamePrefix="select"
              onChange={(selected) =>
                setFilters({ ...filters, categoryIds: selected.map((s) => s.value) })
              }
            />
            <Select
              isMulti
              ...
              classNamePrefix="select"
              onChange={(selected) =>
                setFilters({ ...filters, municipalityIds: selected.map((s) => s.value) })
              }
            />
            <select
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
        <div className="flex flex-col gap-4">
          <h2 className="text-lg font-bold">Pregled izvještaja ({modalFormat.toUpperCase()})</h2>
          {modalFormat === "pdf" ? (
            <iframe src={modalBlobUrl ?? ""} className="w-full h-150" title="PDF preview" />
          ) : (
            <p className="text-gray-700">
              Format {modalFormat.toUpperCase()} je spreman za preuzimanje.
            </p>
          )}
