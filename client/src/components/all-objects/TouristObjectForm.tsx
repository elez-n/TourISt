...
type Props = {
  ...
  refetch?: () => void;
  initialData?: TouristObjectDto;
};

const MAX_IMAGES = 10;
const MAX_FILE_SIZE_MB = 5;

export default function TouristObjectForm({ setEditMode, refetch, initialData }: Props) {
  ...
  const [createTouristObject, { isLoading: isCreating }] = useCreateTouristObjectMutation();
  const [updateTouristObject, { isLoading: isUpdating }] = useUpdateTouristObjectMutation();
  const [deletedPhotographIds, setDeletedPhotographIds] = useState<number[]>([]);
  const [existingPhotos, setExistingPhotos] = useState(initialData?.photographs || []);
  ...
  useEffect(() => {
    ...
    reset({
      ...
      coordinate1: initialData.coordinate1,
      coordinate2: initialData.coordinate2,
    ...
    try {
      ...
      Object.entries(data).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          value.forEach(v => {
            if (v instanceof File) formData.append("photographs", v);
      ...
      deletedPhotographIds.forEach(id => formData.append("deletedPhotographIds", String(id)));
  ...
  return (
    <div className="max-w-6xl mx-auto p-8 bg-white shadow-lg rounded-lg">
      <h2 className="text-3xl font-bold mb-6 text-center">
        {initialData ? "Uredi turistički objekat" : "Unos novog turističkog objekta"}
      </h2>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            />
            {errors.objectTypeId && <p className="text-red-500 text-sm mt-1">{errors.objectTypeId.message}</p>}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div>
            ...
            {errors.numberOfUnits && <p className="text-red-500 text-sm mt-1">{errors.numberOfUnits.message}</p>}
          </div>
          <div>
            <Label>Broj kreveta</Label>
            <Input type="number" {...register("numberOfBeds", { valueAsNumber: true })} />
            {errors.numberOfBeds && <p className="text-red-500 text-sm mt-1">{errors.numberOfBeds.message}</p>}
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              {existingPhotos.map(p => (
                <div key={p.id} className="relative group rounded-md overflow-hidden border">
                  <img src={`https://localhost:5001${p.url}`} className="w-full h-32 object-cover" alt="preview" />
                  <button
                    type="button"
                    onClick={() => {
                      setDeletedPhotographIds([...deletedPhotographIds, p.id]);
                      setExistingPhotos(existingPhotos.filter(ep => ep.id !== p.id));
                    }}
                    className="absolute top-1 right-1 bg-black/70 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition"
                  >
                    ✕
